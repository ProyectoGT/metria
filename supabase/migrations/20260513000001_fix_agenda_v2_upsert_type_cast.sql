-- ─── Fix: cast explicito a integer en upsert_agenda_reminders ──────────────────
--
-- Problema:
--   create_agenda_activity_v2 declara v_result.id y v_empresa_id como BIGINT,
--   pero upsert_agenda_reminders espera INTEGER. PostgreSQL no hace cast
--   implicito de estrechamiento (BIGINT -> INTEGER).
--
--   Error: function upsert_agenda_reminders(integer, date, text, integer, bigint)
--          does not exist
--
-- Solucion: agregar ::integer a los parametros bigint en la llamada.
...
begin
  ...
  -- Registrar recordatorio si aplica (cast explicito a integer)
...
comment on function public.create_agenda_activity_v2 is
  'Crea actividad en agenda (v2). Fix: cast integer en upsert_agenda_reminders.';
