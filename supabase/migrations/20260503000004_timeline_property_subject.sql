-- Permite registrar eventos de timeline asociados solo a una propiedad.
-- Necesario para recomendaciones tipo "revisar propiedad sin actividad".

alter table public.contacto_timeline_events
  drop constraint if exists contacto_timeline_subject_check;

alter table public.contacto_timeline_events
  add constraint contacto_timeline_subject_check check (
    contacto_id is not null
    or pedido_id is not null
    or propiedad_id is not null
  );
...
create or replace function public.can_access_contacto_timeline_event(
  ...
  row_propiedad_id bigint default null
)
returns boolean
language sql
stable
...
as $$
  select case
    ...
    when row_propiedad_id is not null and exists (
      select 1
...
for select
using (
  public.can_access_contacto_timeline_event(empresa_id, contacto_id, pedido_id, agente_id, propiedad_id)
);
