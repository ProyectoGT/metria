-- ─── Fix: usuarios.estado usa 'active' (ingles), no 'activo' (espanol) ──────
-- La columna estado tiene CHECK (estado in ('invited', 'active', 'disabled'))
-- y default 'active'.
-- El RPC create_agenda_activity comparaba con 'activo' (espanol), que nunca
-- coincidia con ningun usuario, causando "Usuario no encontrado o inactivo".
-- ──────────────────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION create_agenda_activity(TEXT, DATE, TEXT, TEXT, TEXT, TEXT, BOOLEAN, TEXT, INTEGER[], TEXT, INTEGER) IS
  'Crea actividad en agenda. Fix: estado = active (no activo).';
