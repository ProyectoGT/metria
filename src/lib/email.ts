...
export async function sendBienvenidaGoogleEmail(params: {
  to: string; nombre: string; correo: string;
}) {
  const html = wrap(`
    ...
    <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#09090b;text-align:center;">Acceso concedido al CRM</h1>
    <p style="margin:0 0 24px;font-size:15px;color:#71717a;text-align:center;line-height:1.6;">Hola <strong>${params.nombre}</strong>, se te ha dado acceso a <strong style="color:#09090b;">Metria CRM</strong> usando tu cuenta de Google <strong>${params.correo}</strong>.</p>
    ${btn(`${BASE_URL}/login`, "Acceder al CRM")}
    ${infoBox("Como acceder", ["1. &nbsp;Haz clic en el boton 'Acceder al CRM'", "2. &nbsp;Selecciona 'Continuar con Google'", `3. &nbsp;Elige la cuenta ${params.correo}`, "4. &nbsp;¡Listo! Ya puedes usar el sistema"])}
    <p style="margin:0;font-size:13px;color:#a1a1aa;text-align:center;">Si tienes algun problema, contacta con el administrador.</p>
  `);
